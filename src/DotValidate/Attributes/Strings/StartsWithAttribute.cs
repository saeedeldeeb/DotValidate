...
/// <summary>
/// Specifies that a string property must start with one of the given values.
/// </summary>
public sealed class StartsWithAttribute : ValidationAttribute
{
    /// <summary>
    /// The values that the string must start with (any one of them).
    /// </summary>
    ...
    protected override string DefaultErrorMessage => "{0} must start with one of the following: {1}.";

    /// <summary>
    /// Initializes a new instance specifying values the string must start with.
    /// </summary>
    /// <param name="values">The values that the string must start with (any one of them).</param>
    public StartsWithAttribute(params string[] values)
    {
